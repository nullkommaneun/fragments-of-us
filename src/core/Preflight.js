...
function icon(status){ return status==='ok' ? '✓' : status==='warn' ? '⚠' : '✖'; }
...
// Laufzeit-Fix, wenn Scroll-Lock fehlt (wirken erst im Spielstart richtig)
  ...
  checks: [
    ...
    { id:'css', label:'Stylesheet (styles.css)', required:true,
      run: async () => {
        ...
        } catch (e) { detail = 'cannot inspect stylesheet: ' + e; }
        // Falls nicht geladen: Critical-CSS greift, aber wir warnen/härten nicht ab
    ...
    // 4) Scroll/Viewport-Guard mit Diagnose + Auto-Fix
    { id: 'scroll', label: 'Scroll-Lock/Viewport', required: true,
      run: async () => {
        ...
        // Auto-Fix anwenden und erneut bewerten
        ...
        const taApp2  = document.getElementById('app') ? css(document.getElementById('app'),'touch-action').trim() : '';
        ...
        const diagAfter = {
          appliedFix: true,
          before: diagBefore,
          after: { overflowRoot:overflowRoot2, overflowBody:overflowBody2, obRoot:obRoot2, obBody:obBody2, touchAction:{root:taRoot2, body:taBody2, app:taApp2} }
        };
...
export async function runPreflight(ctx) {
  ...
  for (const c of PF.checks) {
    try {
      const res = await c.run(ctx); // boolean oder {ok,warn,detail}
