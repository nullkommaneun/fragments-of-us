...
const PF = {
  checks: [
    { id: 'esmodules', label: 'ES Modules', required: true, run: async () => { await import('data:text/javascript,export default 1'); return true; } },
    ...
    { id: 'localstorage', label: 'LocalStorage', required: false, run: async () => { localStorage.setItem('__pf', '1'); localStorage.removeItem('__pf'); return true; } },
    { id: 'audio', label: 'AudioContext', required: false, run: async () => { try { const C = window.AudioContext || window.webkitAudioContext; if (!C) return false; const ac = new C({ latencyHint: 'interactive' }); ac.close(); return true; } catch { return false; } } },
    { id: 'memories', label: 'Daten: memories.json', required: true, run: async () => {
        const r = await fetch('./public/data/memories.json', { cache: 'no-store' }); if (!r.ok) throw new Error(`${r.status} ${r.statusText}`);
        const arr = await r.json(); return Array.isArray(arr) && arr.length >= 1;
      }
    },
  ],
  modules: {
...
export async function runPreflight(ctx) {
  ...
  // 1) Capabilities
  ctx.caps = detectCaps();
  summary.push(row('Geräteprofil', 'ok', `${ctx.caps.webgl2?'WebGL2':'WebGL1'} · ${ctx.caps.deviceMemory}GB · PR ${ctx.caps.pixelRatio.toFixed(2)} · ${ctx.caps.quality}`));
  ...
  // 4) Qualitative Fallback-Anzeige
  if (!ctx.caps.webgl2) {
